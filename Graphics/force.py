...
k = 100
offset = 0.1
simulation = SimulationContainer([100], mass, k, offset, pd.Timedelta(milliseconds=1))
...
for i in range(0, countIterations):
    ...
    forces1.append(simulation.information[1, Enums.FieldStatIndex.ForceX.value])
    forces2.append(simulation.information[2, Enums.FieldStatIndex.ForceX.value])
    forces3.append(simulation.information[3, Enums.FieldStatIndex.ForceX.value])
    forces4.append(simulation.information[4, Enums.FieldStatIndex.ForceX.value])
    forces5.append(simulation.information[5, Enums.FieldStatIndex.ForceX.value])
...
plt.xlabel('Ось t, с')
plt.ylabel('Ось f, Н')
plt.title('График зависимости силы на тело от времени')
plt.legend()
plt.show()
