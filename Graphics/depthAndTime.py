...
def startProgram(countMilliseconds):
    countIterations = int(100000 / countMilliseconds)
    mass = 0.1
    k = 10
    offset = 0.1
    simulation = SimulationContainer([100], mass, k, offset, pd.Timedelta(milliseconds=countMilliseconds), 15)
...
friction = []
for i in range(4, 20):
    friction.append(i * 0.5)
    startProgram(i * 0.5)
plt.plot(friction, ans)
plt.xlabel('Ось t, мс') # время между итерациями
plt.ylabel('Ось n') # номер самого правого грузика, которого задело колебание
plt.title('График зависимости глубины проникновения от времени между итерациями')
plt.show()
