...
simulation = SimulationContainer([200], mass, k, offset_0, pd.Timedelta(milliseconds=0.5), 0.02)
mask = np.full(shape=(202,), fill_value=False, dtype=bool)
mask[50] = True
simulation.setForcedDisplacement(mask, offset_0)
mask = np.full(shape=(202,), fill_value=False, dtype=bool)
mask[150] = True
simulation.setForcedDisplacement(mask, -1 * offset_0)
