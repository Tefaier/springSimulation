...
simulation = SimulationContainer([68], mass, k, offset_0, pd.Timedelta(milliseconds=0.2), 0.1)
simulation.setForcedOscillation([
    OscillationInfo((-1,), offset_0 / 2, calculateNaturalFrequency(mass=mass, k=k), 1),
    OscillationInfo((1,), offset_0 / 2, calculateNaturalFrequency(mass=mass, k=k), -1)
])
